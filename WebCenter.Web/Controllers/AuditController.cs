...
namespace WebCenter.Web.Controllers
{
    public class AuditController : BaseController
    {
        ...
        public ActionResult Search(OrderSearchRequest request)
        {
            ...
            {
                Expression<Func<audit, bool>> tmp = c => (c.customer_id == request.customer_id);
                condition = tmp;
            }
            // 订单状态
            if (request.status != null)
            {
                if (request.status == 2)
                {
                    Expression<Func<audit, bool>> tmp = c => (c.status == 2 || c.status == 3);
                    condition = tmp;
                }
                else
                {
            ...
            {
                Expression<Func<audit, bool>> tmp = c => (c.date_transaction >= request.start_time.Value);
                condition = tmp;
            }
            // 成交结束日期
